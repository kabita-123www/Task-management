...
<html>
	...
	<head>
		<title>Apply Leave | Employee Panel | Task Management System</title>
	...
	<body bgcolor="#F0FFFF">
		...
		$new_date2 = date("Y-m-d", strtotime($date . " +31 days"));
		?>
						...
						<form action="process/applyleaveprocess.php?id=<?php echo $id ?>" method="POST">
							...
							<div class="row row-space">
								<div class="col-2">
									<p>Start Date</p>
									<div class="input-group">
										<input class="input--style-1" min="<?= $new_date ?>" max="<?= $new_date2 ?>"
											type="date" id="start" placeholder="start" name="start">
								...
								<div class="col-2">
									<p>End Date</p>
									<div class="input-group">
										<input class="input--style-1" type="date" id="end" placeholder="end"
											name="end">
		...
		<script>
			...
			document.querySelector("#start").addEventListener("change", function (e) {
				const mindate=e.target.value;
				 ...
				 end.setAttribute("max", maxdate.getFullYear() + "-" + ((maxdate.getMonth()+1 > 9) ? maxdate.getMonth()+1 : "0" + (maxdate.getMonth()+1)) + "-" + ((maxdate.getDate() > 9) ? maxdate.getDate() : "0" + maxdate.getDate()));
